defmodule AmboseliWeb.CommentLive.FormComponent do
  ...
  def handle_event("save", %{"comment" => comment_params}, socket) do
    comment_params =
      case socket.assigns.parent_comment do
        nil ->
          Map.put(comment_params, "notebook_id", socket.assigns.notebook.id)
    ...
    form =
      if comment do
        ...
        if socket.assigns.parent_comment do
          AshPhoenix.Form.for_create(Amboseli.Blog.Comment, :create_child_comment,
            as: "comment",
            actor: socket.assigns.current_user
          )
        else
          AshPhoenix.Form.for_create(Amboseli.Blog.Comment, :create,
            as: "comment",
            actor: socket.assigns.current_user
          )
        end
